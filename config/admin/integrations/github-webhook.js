...
/**
 * Adding a GitHub username to this list will suppress
 * all notifications generated by actions from that user.
 * Helpful for filtering out bot messages.
 */
const ignoredActors = ['renovate[bot]', 'gitpod-io[bot]', 'codesee-architecture-diagrams[bot]'];

/**
 * Adding a branch name to list will suppress all notifications generated by
 * commits to that branch.  NOTE: this will ignore any ref that ends with an 
 * ignored branch name.
 */

const ignoredBranches = ['gh-pages'];
...
const githubEvents = {
  ...
  issues(request) {
    ...
    // Ignore Authors
    if (ignoredActors.includes(user.login)) return;
...
class Script {
  process_incoming_request({ request }) {
    ...
    if (ignoredBranches.some(branch => request?.content?.ref?.endsWith(branch))) {
      return;
    }
